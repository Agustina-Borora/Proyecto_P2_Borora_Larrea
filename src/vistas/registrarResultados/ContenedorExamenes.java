...
/**
 * Un renglón de resultado dentro de la lista de parámetros de un examen (ver cargarReultados):
 * nombre del parámetro, campo para cargar el valor (según tipo_dato: numérico, cualitativo o
 * texto libre), unidad y valor de referencia.
 */
public class ContenedorExamenes extends javax.swing.JPanel {
    ...
    /**
     * Arma el panel (initComponents) y deja listo el renglón vacío: layout del panelCampo donde
     * después se inserta el campo de carga, y los anchos/paddings de las columnas
     * (ajustarProporciones).
     */
    ...
    /**
     * Fija el ancho de cada columna (nombre / campo / unidad / referencia) para que todos los
     * renglones queden alineados, como en una tabla, y les da un poco de aire (padding) para que
     * no se vea amontonado.
     */
    ...
    /**
     * Arma este renglón a partir de un parámetro real: nombre, unidad, referencia y el campo de
     * carga que corresponda según su tipo.
     */
    ...
    /**
     * Para parámetros numéricos: false si el usuario cargó algo que no es un número válido (el
     * renglón ya se pinta de rojo solo, esto es lo que usa cargarReultados para frenar el
     * guardado).
     */
    ...
    /**
     * Vacía panelCampo y le pone el componente de carga que corresponde al tipo_dato del
     * parámetro: "cualitativo" arma un JComboBox con opcionesCualitativo (separadas por coma) más
     * un "Seleccione..." inicial; "texto" arma un JTextArea con scroll para texto libre; cualquier
     * otro valor (incluido null) cae en "numerico", un JTextField con validación en vivo (ver
    ...
    /**
     * Valida en vivo lo que se va tecleando (o se pega) en un campo numérico, en dos niveles: <p>
     * 1) directamente no deja entrar letras ni símbolos que no pintan nada en un número (solo
     * dígitos, coma, punto y signo) -- un "@" ni se escribe; <p> 2) lo que sí entra pero no forma
     * un número válido todavía (un "-" solo, dos puntos, etc.) se sigue marcando en rojo con
     * esValido() = false, hasta que se corrija o se borre.
     */
