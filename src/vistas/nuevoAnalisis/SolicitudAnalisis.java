 ...
 * nomenclador que va a incluir la orden.
 */
public class SolicitudAnalisis extends javax.swing.JPanel {

    /**
     * Prepara el panel: aplica el estilo visual (bordes redondeados), instala el filtro de
     * validación del campo de búsqueda, reemplaza el modelo dummy de la tabla por uno vacío cuya
     * columna "Obra Social" solo es editable con cobertura Mixta, y engancha el botón "Agregar" (y
     * Enter en el campo de búsqueda) a {@link #agregarAnalisis()}.
     */
public SolicitudAnalisis() {
    ...
    ((javax.swing.text.AbstractDocument) jTextField1.getDocument()).setDocumentFilter(new vistas.panels.FiltroLetraONumero());

    // Reemplaza las filas dummy del editor visual por un modelo real y vacío.
    // La columna "Obra Social" solo es editable por fila cuando la cobertura es Mixto.
...
/**
 * Tipo de cobertura elegido en la pantalla (lo manda TipoCobertura via listener).
 */
...
/**
 * "No" si es Particular, "Si" en cualquier otro caso (Obra Social fijo en Si, Mixto arranca en
 * Si y se puede editar).
 */
...
/**
 * Se llama después de generar una orden con éxito: vacía la tabla y la lista para la próxima
 * orden.
 */
...
/**
 * Se llama desde NuevoAnalisis cada vez que cambia la tarjeta elegida en Tipo de Cobertura
 * ("PARTICULAR", "OBRA_SOCIAL" o "MIXTO").
 * Actualiza la columna "Obra Social" de todas las filas ya agregadas al nuevo valor por
 * defecto y deja la columna editable o no segun corresponda (solo Mixto permite tocarla a
 * mano).
 */
